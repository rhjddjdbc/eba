...
static void disassemble_section(ElfContext *ctx, Elf64_Shdr *sec, const char *secname,
                                const char *output_dir, FILE *full_out, FILE *json_out,
                                int *first_sec_json, int *any_section) {
    if (sec->sh_size == 0 || sec->sh_addr == 0) return;
    ...
    *any_section = 1;
    ...
    for (size_t j = 0; j < count; j++) {
        ...
        if (strcmp(insn[j].mnemonic, "ret") == 0 ||
            ...
            if (func_file) { fclose(func_file); func_file = NULL; }
        }
    }
...
void disassemble_full_to_file(ElfContext *ctx, AnalyzerConfig *cfg, FILE *json_out) {
    ...
    for (int i = 0; i < ctx->ehdr.e_shnum; i++) {
        ...
        if (cfg->disasm_all)
            do_disasm = 1;
        else if (strcmp(secname, ".init") == 0 && cfg->disasm_init)
            do_disasm = 1;
        else if (strcmp(secname, ".fini") == 0 && cfg->disasm_fini)
            do_disasm = 1;
        else if (strstr(secname, ".text") != NULL && cfg->disasm_text)
            do_disasm = 1;
        else if (strstr(secname, ".plt") != NULL && cfg->disasm_plt)
            do_disasm = 1;
        else if (strcmp(secname, ".got") == 0 && cfg->disasm_got)
            do_disasm = 1;
        else if (strcmp(secname, ".rodata") == 0 && cfg->disasm_rodata)
            do_disasm = 1;
        else if (strcmp(secname, ".data.rel.ro") == 0 && cfg->disasm_data_rel_ro)
            do_disasm = 1;
        else if (strcmp(secname, ".eh_frame") == 0 && cfg->disasm_eh_frame)
            do_disasm = 1;
        else if (strcmp(secname, ".init_array") == 0 && cfg->disasm_init_array)
            do_disasm = 1;
        else if (strcmp(secname, ".fini_array") == 0 && cfg->disasm_fini_array)
            do_disasm = 1;

        if (!do_disasm) continue;

        printf("Disassembling: %s (%zu bytes)\n", secname, sec->sh_size);
        disassemble_section(ctx, sec, secname, output_dir, full_out, json_out,
                            &first_sec_json, &any_section);
    }
