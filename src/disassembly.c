...
void disassemble_full_to_file(ElfContext *ctx, AnalyzerConfig *cfg, FILE *json_out)
{
    ...
    strncpy(output_dir, cfg->output_dir, sizeof(output_dir) - 1);
    ...
    for (int i = 0; i < ctx->ehdr.e_shnum; i++) {
        ...
        // Section file - remove leading dot from section name for filename
        char sec_filename[256];
        if (secname[0] == '.') {
            strncpy(sec_filename, secname + 1, sizeof(sec_filename) - 1);
        } else {
            strncpy(sec_filename, secname, sizeof(sec_filename) - 1);
        }
        sec_filename[sizeof(sec_filename) - 1] = '\0';
        ...
        for (size_t j = 0; j < count; j++) {
            ...
            if (strcmp(insn[j].mnemonic, "push") == 0 && strstr(insn[j].op_str, "rbp")) is_start = 1;
            else if (strcmp(insn[j].mnemonic, "endbr64") == 0) is_start = 1;
            else if (strcmp(insn[j].mnemonic, "sub") == 0 && strstr(insn[j].op_str, "rsp")) is_start = 1;
